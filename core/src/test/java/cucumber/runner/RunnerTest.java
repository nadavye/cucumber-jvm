...
public class RunnerTest {
    ...
    public void hooks_execute_when_world_exist() throws Throwable {
        ...
        runner.runPickle(createEmptyPickle(), mockDialect());
    ...
    public void steps_are_skipped_after_failure() throws Throwable {
        ...
        runner.runPickle(createPickleMatchingStepDefinitions(asList(stepDefinition)), mockDialect());

        InOrder inOrder = inOrder(failingBeforeHook, stepDefinition);
        inOrder.verify(failingBeforeHook).execute(Matchers.<Scenario>any());
        inOrder.verify(stepDefinition, never()).execute(Matchers.<GherkinDialect>any(), Matchers.<Object[]>any());
    }
    ...
    private GherkinDialect mockDialect() {
        GherkinDialect dialect = mock(GherkinDialect.class);
        when(dialect.getLanguage()).thenReturn("en");
        return dialect;
    }
}
