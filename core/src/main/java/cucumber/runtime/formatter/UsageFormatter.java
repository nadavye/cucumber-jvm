...
class UsageFormatter implements Formatter {
    ...
    void handleTestStepResult(TestStepFinished event) {
        if (event.result.getStatus().equals(Result.PASSED)) {
            addUsageEntry(event.result, event.testStep.getPattern(), event.testStep.getStepText(), event.testStep.getLocation());
        }
    }
